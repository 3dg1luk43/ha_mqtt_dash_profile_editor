...
export default function DeviceSelector() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px', background: '#f7f9fc', borderBottom: '1px solid #e8eaed' }}>
      ...
      {device.key !== 'custom' && (
        <span style={{ fontSize: 11, color: '#888' }}>
          {device.points[0]}×{device.points[1]}pt · {device.note}
        </span>
      )}
      ...
      {device.key === 'custom' && (
        <>
          ...
          <input
            ...
            placeholder="Height"
          />
          <span style={{ fontSize: 11, color: '#888' }}>pt</span>
        </>
      )}
    </div>
  );
